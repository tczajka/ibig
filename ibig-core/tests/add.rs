...
use ibig_core::{
    Digit, IDigit, add_digit_idigit, add_idigit_idigit, add_signed_digit, add_signed_icarry,
    add_signed_idigit, add_signed_signed, add_signed_unsigned, add_unsigned_1, add_unsigned_carry,
    add_unsigned_digit, add_unsigned_icarry, add_unsigned_idigit, add_unsigned_signed,
...
#[test]
fn add_signed_icarry_basic() {
    // -1 + 1 == 0.
    let mut a = [Digit::MAX];
    assert_eq!(add_signed_icarry(&mut a, idigit(1)), idigit(0));
    assert_eq!(a, [Digit::ZERO]);

    // 0 + -1 == -1, borrowing out the top.
    let mut a = [Digit::ZERO, Digit::ZERO];
    assert_eq!(add_signed_icarry(&mut a, idigit(-1)), idigit(-1));
    assert_eq!(a, [Digit::MAX, Digit::MAX]);

    // Carry 0 leaves the slice unchanged.
    let mut a = [digit(7), digit(2)];
    assert_eq!(add_signed_icarry(&mut a, idigit(0)), idigit(0));
    assert_eq!(a, [digit(7), digit(2)]);

    // The signed max plus 1 becomes the sign bit, with a 0 sign carry.
    let signed_max = Digit::MAX >> 1;
    let mut a = [signed_max];
    assert_eq!(add_signed_icarry(&mut a, idigit(1)), idigit(0));
    assert_eq!(a, [signed_max + digit(1)]);
...
#[test]
#[should_panic]
fn add_signed_icarry_empty() {
    add_signed_icarry(&mut [], idigit(1));
}
...
proptest! {
    ...
    #[test]
    fn add_signed_digit_matches_signed_unsigned(a in vec(any::<Digit>(), 1..20), d: Digit) {
        let mut via_digit = a.clone();
        let mut via_slice = a;
        let high_digit = add_signed_digit(&mut via_digit, d);
        let high_slice = add_signed_unsigned(&mut via_slice, &[d]);
        prop_assert_eq!(via_digit, via_slice);
        prop_assert_eq!(high_digit, high_slice);
    }
    ...
    // `add_signed_icarry(a, c)` (c in -1..=1) agrees with `add_signed_signed` of the slice `[c]`.
    #[test]
    fn add_signed_icarry_matches_signed_signed(a in vec(any::<Digit>(), 1..20), c in -1i8..=1) {
        let carry = idigit(c);
        let mut via_icarry = a.clone();
        let mut via_slice = a;
        let high_icarry = add_signed_icarry(&mut via_icarry, carry);
        let high_slice = add_signed_signed(&mut via_slice, &[carry.cast_unsigned()]);
        prop_assert_eq!(via_icarry, via_slice);
        prop_assert_eq!(high_icarry, high_slice);
    }
